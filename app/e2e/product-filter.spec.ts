...
test.describe("Product filter", () => {
  ...
  test("URL override with ?products=", async ({ page }) => {
    await page.goto("/?products=coderabbit,copilot");
    await expect(
  ...
  test("unfiltered sections remain unfiltered", async ({ page }) => {
    await page.goto("/?products=coderabbit,copilot");
  ...
  test("bot detail page unaffected by filter", async ({ page }) => {
    // Set filter to exclude coderabbit via URL
    await page.goto("/?products=copilot");
    await expect(
      page.getByTestId("product-filter-bar").getByText(/Showing 1 of/),
    ).toBeVisible();
